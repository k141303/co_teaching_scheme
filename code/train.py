...
from data.datasets import load_shinra_dataset
...
def train(cfg, dataset, dev_dataset, model, writer):
    ...
    for epoch in range(1,cfg.train.epoch+1):
        ...
        score = dataset.evaluate(train_outputs)
...
def main(cfg):
    ...
    set_seed(cfg.seed)
    train_dataset, dev_dataset = load_shinra_dataset(cfg)

    if cfg.model.output_dir is not None:
        output_dir = hydra.utils.to_absolute_path(cfg.model.output_dir)
        os.makedirs(output_dir, exist_ok=True)
        train_dataset.save_pageids(os.path.join(output_dir, "train_pageids.json"))
    ...
    if cfg.model.is_distilled:
        _, config, model = BertForDistillingIOB2Tagging.load(
            model_dir,
            config_name=cfg.model.config_name,
            model_name=cfg.model.model_name
        )
        config.hidden_dropout_prob=cfg.train.hidden_dropout_prob
        config.attention_probs_dropout_prob=cfg.train.attention_probs_dropout_prob
        bert = model.bert
    else:
